...
export function StylishTemplate({ data }: StylishTemplateProps) {
    ...
    <div className="w-full h-full bg-white text-black overflow-auto">
      <div className="grid grid-cols-12 min-h-full">
        ...
        <aside className="col-span-4 p-3 bg-[#f5f3f0] border-r border-[#8b7d6b]">
          <div className="-mx-3 -mt-3 mb-1.5 px-3 pt-2 pb-3 bg-[#e8e3dd] border-b border-[#8b7d6b]">
            ...
            {occupation ? (
              <div className="mt-0.5 text-[7px] font-normal text-black">
                {occupation}
              </div>
            ) : null}
          </div>
            ...
            <SectionBlock title="SKILLS" panelTone="secondary-dark">
              <div className="space-y-1">
                {data.skills.slice(0, 10).map((skill, idx) => (
                  <SkillBarRow key={idx} name={skill} level="Proficient" />
                ))}
              </div>
            </SectionBlock>
          )}
        ...
        <main className="col-span-8 p-3">
          ...
          {data.summary && (
            <SectionBlock title="PROFILE">
              <p className="text-[7px] leading-tight text-black text-justify">{data.summary}</p>
            </SectionBlock>
          )}
                ...
                {data.experience.map((exp) => (
                  <div key={exp.id}>
                    <div className="flex justify-between items-baseline gap-2">
                      ...
                      {exp.location && <span className="text-[7px] text-black shrink-0">{exp.location}</span>}
                    </div>
                    <div className="text-[7px] text-black mt-0">
                      {exp.startDate} - {exp.current ? 'Present' : exp.endDate}
                    </div>
                    {exp.description && (
                      <div className="mt-1 text-[7px] leading-tight text-black">
                ...
                {data.education.map((edu) => (
                  <div key={edu.id}>
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="text-[7px] text-black shrink-0">
                        {edu.startDate} - {edu.endDate}
                      </span>
                    </div>
                    {edu.location && (
                      <div className="text-[7px] text-slate-600 mt-0">{edu.location}</div>
                    )}
                  </div>
                ))}
              </div>
                ...
                {data.projects.map((proj) => (
                  <div key={proj.id}>
                    <div className="flex justify-between items-baseline gap-2">
                      ...
                      {proj.link && (
                        <a href={proj.link} target="_blank" rel="noreferrer" className="text-[7px] text-slate-600 underline shrink-0">
                          Link
                        </a>
                      )}
                    </div>
                    {proj.technologies && proj.technologies.length > 0 && (
                      <div className="text-[7px] text-slate-600 mt-0">{proj.technologies.join(', ')}</div>
                    )}
                    {proj.description && (
                      <div className="mt-1 text-[7px] leading-tight text-black">{proj.description}</div>
                    )}
                  </div>
                ))}
              </div>
...
function SectionBlock({
  ...
  return (
    <section className="mb-1.5 last:mb-0">
      <h2 className="text-[7px] font-bold uppercase tracking-wide text-[#8b7d6b]">{title}</h2>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="mb-1.5">
      <span className="text-[7px] font-bold text-black block">{label}</span>
      <span className="text-[7px] text-black block mt-0">{value || '—'}</span>
    </div>
  );
}
...
function SkillBarRow({ name, level }: { name: string; level: string }) {
  const pct = levelToPercent(level);
  return (
    <div>
      <div className="flex items-center justify-between gap-2 mb-0.5">
        <span className="text-[7px] text-black flex-1">{name}</span>
