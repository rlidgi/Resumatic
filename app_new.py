...
def plans_template_pdf():
    ...
    safe_next_url = next_url if (
            next_url and _is_safe_next_url(next_url)) else ''
    flash('A Plan is required for PDF downloads', 'warning')
    if safe_next_url:
        return redirect(url_for('plans', next=safe_next_url))
...
def api_template_pdf(template_id):
    ...
    try:
        ...
        if not paid_flag:
            if not _ON_AZURE:
                logger.warning(
                    "template_pdf local-dev paid bypass user_id=%s email=%s t=%sms",
                    user_id_for_log,
                    user_email_for_log,
                    _t(),
                )
