...
def plans():
    ...
    raw_plans = _filter_stripe_plans_for_region(
        _get_active_stripe_plans(force_refresh=True, blocking=False)
    )
...
def _redirect_to_stripe_payment_link(
    ...
    logger.info(
        "[PAYMENT LINK] Entered requested plan_id=%s",
        plan_id,
    )
    plan_id = _normalize_plan_id(plan_id)

    logger.info(
        "[PAYMENT LINK] requested plan_id=%s",
        plan_id,
    )
    ...
    logger.info(
        "[PAYMENT LINK] resolved link=%r",
        link,
    )

    if not link:
        logger.info("[PAYMENT LINK] no payment link found")
        return None
    params = {}
    try:
...
def checkout():
    ...
    checkout_trace = str(uuid.uuid4())[:8]

    logger.info(
        "[CHECKOUT %s] START user=%s authenticated=%s",
        checkout_trace,
    ...
    logger.info(
        "[CHECKOUT %s] RAW PARAMS plan_arg=%r invite=%r offer_retention=%s reinstate_offer=%s",
        checkout_trace,
        plan_arg,
        invite_token,
        offer_retention,
    ...
    logger.info(
        "[CHECKOUT %s] NORMALIZED plan_arg=%r -> plan_id=%r",
        checkout_trace,
        plan_arg,
        plan_id,
    )
    ...
    if not plan:
        ...
        logger.info(
            "[CHECKOUT %s] PLAN LOOKUP result=%s",
            checkout_trace,
            {
                "plan_id": plan_id,
                "price_id": plan.get("price_id") if plan else None,
                "role": plan.get("role") if plan else None,
                "label": plan.get("label") if plan else None,
                "interval": plan.get("interval") if plan else None,
                "trial_fee_price_id": plan.get(
                    "trial_fee_price_id"
                    ) if plan else None,
            },
        )
    ...
    logger.info(
        "[CHECKOUT %s] PRICE RESOLUTION plan_id=%s resolved_price_id=%s role=%s",
        checkout_trace,
        plan_id,
        price_id,
        plan_role,
    ...
    if _stripe_enabled() and not _is_email_trial_plan(plan_id):
        ...
        if is_standard_trial:
            logger.info(
                "[CHECKOUT %s] TRY PAYMENT LINK plan_id=%s price_id=%s role=%s",
                checkout_trace,
                plan_id,
                price_id,
                plan_role,
            ...
            logger.info(
                "[CHECKOUT %s] PAYMENT LINK RESULT redirected=%s",
                checkout_trace,
                bool(pl_redirect),
            )
            if pl_redirect:
