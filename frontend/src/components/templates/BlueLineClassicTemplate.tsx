...
export default function BlueLineClassicTemplate({
    content,
    editMode = false,
    onContentChange,
}: {
            ...
            <div className="col-span-9">
                ...
                {editMode ? (
                    <div className="text-[12px] font-bold text-black/80">
                        <EditableText value={String(company)} onChange={(v) => onUpdate(idx, 'company', v)} editMode={editMode} liveUpdate layoutSafe className="inline" as="span" />
                        {(String(company).trim() && String(location).trim()) ? <span> — </span> : null}
                        <EditableText value={String(location)} onChange={(v) => onUpdate(idx, 'location', v)} editMode={editMode} liveUpdate layoutSafe className="inline" as="span" />
                    </div>
                ) : (
