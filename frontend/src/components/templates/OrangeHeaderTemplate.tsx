...
export default function OrangeHeaderTemplate({
    content,
    editMode = false,
    onContentChange,
}: {
    ...
    return (
        <div>
            <div className="flex items-baseline justify-between gap-4">
                ...
                {dates ? (
                    editMode ? (
                        <EditableText value={String(dates)} onChange={(v) => onUpdate(idx, 'duration', v)} editMode={editMode} liveUpdate layoutSafe as="div" className="text-sm text-black/70 whitespace-nowrap" />
                    ) : (
                        <div className="text-sm text-black/70 whitespace-nowrap">{String(dates)}</div>
                    )
                ) : null}
            </div>
            {(company || location) && (
                <div className="text-sm text-black/80 font-semibold">
                    {editMode ? (
                        <>
                            <EditableText value={String(company)} onChange={(v) => onUpdate(idx, 'company', v)} editMode={editMode} liveUpdate layoutSafe as="span" className="inline" />
                            {location ? <span className="font-semibold text-black/70"> – </span> : null}
                            <EditableText value={String(location)} onChange={(v) => onUpdate(idx, 'location', v)} editMode={editMode} liveUpdate layoutSafe as="span" className="inline font-semibold text-black/70" />
                        </>
                    ) : (
                        <>
                            {String(company)}
