            ...
            <div className="col-span-11">
                <div className="flex items-start justify-between gap-4">
                    <div className="font-semibold text-[13px]">
                        {editMode ? (
                            <>
                                <EditableText value={String(title || '')} placeholder="Role" onChange={(v) => onTitleChange && onTitleChange(v)} editMode={!!editMode} liveUpdate layoutSafe as="span" className="inline" />
                                <span className="font-normal text-black/70"> — </span>
                                <EditableText value={String(subtitle || '')} placeholder="Company" onChange={(v) => onSubtitleChange && onSubtitleChange(v)} editMode={!!editMode} liveUpdate layoutSafe as="span" className="inline font-normal text-black/70" />
                            </>
                        ) : (
                            <>
                                {title}
                    ...
                    {editMode ? (
                        <EditableText value={String(dates || '')} placeholder="Dates" onChange={(v) => onDatesChange && onDatesChange(v)} editMode={!!editMode} liveUpdate layoutSafe as="div" className="text-[11px] text-black/70 whitespace-nowrap" />
                    ) : (
                        dates ? <div className="text-[11px] text-black/70 whitespace-nowrap">{dates}</div> : null
                    )}
                </div>
                {body ? (
                    <div className="mt-1.5">
                        {editMode ? (
                            <EditableText value={String(body)} placeholder="Add bullets or a short description" onChange={(v) => onBodyChange && onBodyChange(v)} editMode={!!editMode} liveUpdate layoutSafe as="div" className="text-[11px] leading-relaxed text-black/80" multiline />
                        ) : (
                            <RenderMaybeBullets
                                text={body}
                                forceBullets
